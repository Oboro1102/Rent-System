            ...
            <div class="form__row__column">
              <div class="form__content">
                <h4 class="form__content__title">
                  承租人
                  <button type="button" class="button button--combobox" @click="callComboBox('lessee')">
                ...
                <div class="form__selector">
                  <div class="form__selector__result" v-if="rentData.lessee">
                    <img :src="rentData.lessee.picture.thumbnail" :alt="rentData.lessee.name.last" />
                    <span class="combobox__dropdowns__item__tag textColor--blue"
                      v-if="rentData.lessee.gender === 'male'">
                      <i class="fas fa-mars"></i>
                    </span>
                    <span class="combobox__dropdowns__item__tag textColor--pink" v-else>
                      <i class="fas fa-venus"></i>
                    </span>
                    <span v-text="`${rentData.lessee.name.first}${rentData.lessee.name.last}`" />
                  </div>
                </div>
                <ComboBox ref="lessee" :dropdownsData="members" placeholder="快速搜尋請輸入聯絡電話" searchBy="phone"
                  @selectedValue="updateSelectedValue('lessee', $event)">
                  <template #dropdowns="dropdowns">
                    <img :src="dropdowns.item.picture.thumbnail" :alt="dropdowns.item.name.last" />
                    <span class="combobox__dropdowns__item__tag textColor--blue"
                    ...
                    <span v-text="dropdowns.item.name.last" />
                  </template>
                </ComboBox>
                <span class="form__content__alert" v-if="alert.lessee" v-text="alert.lessee" />
              </div>
              <div class="form__content">
                <h4 class="form__content__title">
                  租借品
                  <button type="button" class="button button--combobox" @click="callComboBox('purchase')">
                    <span v-if="rentData.purchase.length < 1">追加</span>
                    <span v-else>選擇租借物件</span>
                  </button>
                </h4>
                <div class="form__selector">
                  <div class="form__selector__result" v-for="(item, index) in rentData.purchase" :key="item.id">
                    ...
                    <div class="form__selector__result__select">
                      <select name="qty" v-model="rentData.purchase[index].qty">
                        <option v-for="qty in Number(item.stock)" :key="qty" :value="qty" v-text="qty" />
                      </select>
                    </div>
                  </div>
                </div>
                <ComboBox ref="purchase" :dropdownsData="goods" placeholder="快速搜尋請輸入物件名稱" searchBy="title"
                  @selectedValue="updateSelectedValue('purchase', $event)">
                  <template #dropdowns="dropdowns">
                    <img :src="dropdowns.item.pic" :alt="dropdowns.item.title" />
                    <span v-text="dropdowns.item.title" />
                    <span class="combobox__dropdowns__item__notice" v-text="`剩餘：${dropdowns.item.stock}`" />
                  </template>
                </ComboBox>
                <span class="form__content__alert" v-if="alert.purchase" v-text="alert.purchase" />
              </div>
            </div>
            <div class="form__row__column">
              <div class="form__content">
                <h4 class="form__content__title">歸還日期</h4>
                <input type="date" name="returnDate" v-model="rentData.returnDate" />
                <span class="form__content__alert" v-if="alert.returnDate" v-text="alert.returnDate" />
              </div>
              <div class="form__content">
                <h4 class="form__content__title">備註</h4>
                <textarea name="note" rows="3" placeholder="請輸入內容" v-model="rentData.notes" />
              </div>
            </div>
          </div>
  ...
  setup() {
    ...
    const sendData = () => {
      ...
      const data = {
        ...
        created: `${new Date().toLocaleDateString("zh-TW").replace(/\//g, "-")} ${new Date().toLocaleTimeString("zh-TW", {
          hour12: false,
          hour: "2-digit",
          minute: "2-digit",
        })}`
